...
if __name__ == "__main__":
    for i in range(1):
        ...
        keys = random.sample(range(N), 1000)
    ...
    with open('test_LSM.txt', 'a') as file: 
        file.write(f"ROWS \n{N} VALUE_SIZE {VALUE_SIZE} SELECT 1000 current time: {ct}")
        file.write(f"\nczas_Insert_LSM = {czas_Insert}")
        file.write(f"\nczas_avg_Insert_LSM = {czas_avg_Insert}")
        file.write(f"\nczas_Select_LSM = {czas_Select}")
        file.write(f"\nczas_avg_Select_LSM = {czas_avg_Select}")
